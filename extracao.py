...
tempoinicio = ''
gerarboleto = False
salvardadospdf = False
resposta = ''


def controlaextracao(visual):

    tempoinicio = time.time()

    if visual.var1 is not None:
        gerarboleto = not visual.var1.get()

    if visual.var2 is not None:
    ...
    if indicecliente == '0000':
        resultado = bd.consultar(senha.sqlcbm)
    else:
        resultado = bd.consultar(senha.sqlcbm.replace(';', ' ') + "WHERE Codigo >= '{codigo}'".format(codigo=indicecliente))

    bd.fecharbanco()

    if visual.var2 is None:
        extrairboletosbombeiro(visual)
    else:
        extrairboletosiptu(visual)

def extrairboletosbombeiro(visual):
    """
    ...
    try:
        ...
        for indice, linha in enumerate(resultado):
            ...
            if aux.hora('America/Sao_Paulo', 'HORA') < datetime.time(22, 00, 00) and texto != 'Este serviço encontra-se temporariamente indisponível.':
                ...
                cbm = '{}{}{}{}{}{}{}-{}'.format(*cbm)
                visual.mudartexto('labelinscricao', 'Inscrição: ' + cbm)
...
def extrairboletosiptu(visual):
    ...
    try:
        ...
        resposta = str(visual.radio_valor.get())
        listaiptucompleta = messagebox.msgbox('Deseja extrair somente os IPTUs faltantes?', messagebox.MB_YESNO, 'Somente Faltantes')
        if listaiptucompleta == messagebox.IDNO:
            sqlaexecutar = senha.sqliptucompleto
        else:
            sqlaexecutar = senha.sqliptufaltante
        ...
        if indicecliente == '0000':
            resultado = bd.consultar(sqlaexecutar)
        else:
            resultado = bd.consultar(sqlaexecutar.replace(';', ' ')+"WHERE Codigo >= '{codigo}'".format(codigo=indicecliente))
        ...
        for indice, linha in enumerate(resultado):
            ...
            iptu = '{}.{}{}{}.{}{}{}-{}'.format(*iptu)
            visual.mudartexto('labelinscricao', 'Inscrição Imobiliária: '+iptu)
