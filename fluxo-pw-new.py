...
def efetuar_logout(page, site_url: str) -> None:
    ...
    print("🔄 [LOGOUT] Iniciando procedimento...")
        ...
        try:
            ...
            if not botao_sair.is_visible():
                ...
                if menu_avatar.is_visible():
                    print("   🖱️ Abrindo menu suspenso (Imodata)...")
                    menu_avatar.click()
                    page.wait_for_timeout(800)

            if botao_sair.is_visible(timeout=3000):
                print("   🖱️ Clicando em 'Sair' (Imodata)...")
                botao_sair.click()
                page.wait_for_timeout(1500)

                # 2. CLICAR EM NOVO LOGIN
            print("   🔍 Procurando botão 'Novo Login'...")
            ...
            if btn_novo.is_visible(timeout=3000):
                print("   🖱️ Botão visível, clicando...")
                btn_novo.click(force=True)
                page.wait_for_timeout(1000)
                sucesso_reset = True
            else:
                # Se não clicou, injeta JS
                print("   💉 Injetando JS para restaurar formulário...")
                page.evaluate("""() => {
                    try {
            ...
            if sucesso_reset:
                print("   ✅ Formulário de login restaurado. Mantendo página atual.")
                return

        except Exception as e:
            print(f"   ⚠️ Erro Imodata Logout: {e}")
    ...
    if "cipafacil" in url_lower:
        try:
            ...
            try:
                ...
                print(f"   ⚠️ Não conseguiu verificar usuário logado: {e}")
            # ===== FIM VERIFICAÇÃO ===== ↑
            ...
            if usuario_correto:
                print(f"   ✅ Usuário CORRETO ({usuario_logado})")
                ...
                try:
                    ...
                    if menu_condo.is_visible(timeout=2000):
                        print(f"   🏢 Dentro de condomínio - voltando para área de seleção...")
                        ...
                        if btn_sair_condo.is_visible(timeout=3000):
                            ...
                            page.wait_for_timeout(1500)
                            print(f"   ✅ Voltou para área de condomínios")
                            return  # ← PARA AQUI! Não faz logout do usuário

                    else:
                        print(f"   ℹ️  Já está na área de condomínios")
                        return  # ← JÁ ESTÁ NO LUGAR CERTO!

                except Exception as e:
                    print(f"   ⚠️ Erro ao voltar para área de condomínios: {e}")
                # ===== FIM ESTRATÉGIA A ===== ↑
                ...
                try:
                    ...
                    if menu_condo.is_visible(timeout=2000):
                        print(f"   🏢 Saindo do condomínio...")
                        menu_condo.click(timeout=5000)
                        page.wait_for_timeout(1000)

                        page.get_by_role("menuitem", name="Sair").click(timeout=5000)
                        page.wait_for_timeout(1500)
                        print(f"   ✅ Saiu do condomínio")
                except Exception as e:
                    print(f"   ℹ️  Não estava em condomínio: {e}")

                # Nível 2: Logout do usuário
                try:
                    print(f"   👤 Fazendo logout do usuário...")
                    ...
                    print(f"   ⚠️ Erro no logout do usuário: {e}")
                # ===== FIM ESTRATÉGIA B ===== ↑

        except Exception as e:
            print(f"   ⚠️ Erro CIPA Logout: {e}")
    # ===== FIM CIPA ===== ↑
...
def _forcar_home(page, url: str) -> None:
    ...
    print(f"   🏠 Voltando para página inicial...")
...
class ExecutorFluxoPW:
        ...
        for i, passo in enumerate(passos):
            try:
                ...
                if skip_mode == "SKIP_LOGIN" and scope == "login":
                    print(f"[SKIP] Passo {i + 1} (login) - sessão reutilizada")
                    continue

                if skip_mode == "SKIP_TO_BOLETOS":
                    # Pula até chegar no clique de "Boleto"
                    if action not in ["click"] or "boleto" not in str(passo.get("locator", {})).lower():
                        print(f"[SKIP] Passo {i + 1} - indo direto para boletos")
                        continue
                    else:
                        skip_mode = None  # Para de pular

                print(f"[PASSO {i + 1}/{len(passos)}] action={action} scope={scope}")
                ...
                if "SKIP_LOGIN" in msg:
                    print(f"♻️  [SESSÃO] Reutilizando login - pulando passos de autenticação")
                    skip_mode = "SKIP_LOGIN"
                    continue
                elif "SKIP_TO_BOLETOS" in msg:
                    print(f"♻️  [SESSÃO] Reutilizando tudo - pulando para boletos")
                    skip_mode = "SKIP_TO_BOLETOS"
                    continue

                elif "SKIP_FLOW" in msg:
                    print(f"⚠️ [SINALIZADOR] Passo opcional falhou. Pulando restante do fluxo desta linha...")
                    skip_mode = "SKIP_FLOW"
    ...
    def _verificar_usuario_ativo(self, step, contexto):
        ...
        print(f"   🕵️ [SMART LOGIN] Verificando sessão ativa...")
        ...
        try:
            ...
            if loc_email.is_visible(timeout=3000):
                ...
                print(f"       👤 Usuário logado detectado: {email_tela}")

                if usuario_esperado in email_tela:
                    print(f"       ✅ Usuário CORRETO! Mantendo sessão e pulando login.")
                    return "BREAK_FLOW"
                else:
                    print(f"       ⛔ Usuário INCORRETO (Esperado: {usuario_esperado})...")

                    # 2. TENTA LOGOUT VIA UI (Visual)
                    try:
                        if selector_menu:
                            print("       🖱️ Abrindo menu de perfil...")
                        ...
                        print("       🖱️ Clicando em Sair...")
                        self.page.locator(self._locator_from_json(selector_logout)).first.click(force=True)
                        self.page.wait_for_timeout(1000)
                    except Exception as e:
                        print(f"       ⚠️ Falha no clique de logout visual: {e}")

                    # 3. LOGOUT NUCLEAR (Limpa Cookies)
                    # Isso garante que a sessão morra, mesmo se o clique falhar
                    print("       🛡️ Forçando limpeza de cookies para garantir logout...")
                    self.page.context.clear_cookies()

                    return None  # Segue para o login
            else:
                print("       ⚪ Nenhuma sessão ativa visível. Seguindo para login...")
                return None

        except Exception as e:
            print(f"       ⚠️ Erro na verificação de sessão: {e}")
            return None
    ...
    def _remover_elemento_bloqueante(self, selector):
        ...
        print(f"   💣 [NUCLEAR] Removendo elemento bloqueante: {selector}")
        try:
            ...
            print(f"   ⚠️ Erro ao remover elemento: {e}")
    ...
    def _baixar_com_prioridade(self, page_like, specs_em_ordem: list, destino: Path, timeout_ms: int = 20000):
        ...
        try:
            ...
            if erro_el.is_visible(timeout=2000):
                ...
                print(f"⚠️ [ERRO NO POPUP] {texto_erro}")
        ...
        for spec in specs_em_ordem or []:
            ...
            try:
                ...
                self.baixados += 1
                print(f"   ✅ Download realizado com sucesso: {destino.name}")
                return destino
    ...
    def _expandir_lista(self, locator_spec: dict, timeout=1500, max_clicks=999, settle_ms=800):
        ...
        print(f"[expandir_lista] cliques={clicks} via={used}")
    ...
    def _verificar_erro_login(self, locator_spec: dict, timeout: int = 3000,
        ...
        try:
            ...
            print(f"❌ [ERRO LOGIN DETECTADO] {texto_erro}")
            ...
            if deve_parar:
                ...
                print(f"🔄 [LIMPANDO ESTADO] Fazendo logout após erro de login...")
                try:
                    ...
                    print(f"✅ [ESTADO LIMPO] Logout realizado")
                except Exception as e:
                    print(f"⚠️ [LOGOUT] Erro ao limpar estado: {e}")
                # ===== FIM ===== ↑
            ...
            if self.login_ok is None:
                self.login_ok = True
            print(f"✅ [LOGIN OK] Nenhum erro detectado")
            return False, None
    ...
    def _fuzzy_lista_fechada(self, alvo: str, container_spec: dict, items_css: str = "a",
        ...
        if toggle_spec:
            ...
            try:
                ...
                print(f"  ✅ Clicou em '{best_txt}' (clique normal)")
            except Exception as e1:
                print(f"  ⚠️  Clique normal falhou: {e1}")
                try:
                    elemento.click(force=True, timeout=5000)
                    print(f"  ✅ Clicou em '{best_txt}' (force click)")
                except Exception as e2:
                    print(f"  ⚠️  Force click falhou: {e2}")
                    try:
                        elemento.evaluate("el => el.click()")
                        self.page.wait_for_timeout(800)
                        print(f"  ✅ Clicou em '{best_txt}' (JavaScript)")
                    except Exception as e3:
                        print(f"  ❌ Todas as 3 estratégias falharam!")
        ...
        for k, w in enumerate(winners):
            ...
            try:
                ...
                try:
                    ...
                    aux.adicionarcabecalhopdf_topo_adaptativo(str(destino), str(destino), codigo, senha_base=self.cpf_senha)
                    print(f"✅ [CABEÇALHO] Código {codigo} adicionado")
                except Exception as e:
                    print(f"⚠️ [CABEÇALHO] Erro: {e}")
                # ===== FIM ===== ↑

                # ===== PROCESSA BOLETO E MOVE PARA PASTA DO MÊS ===== ↓
                try:
                    import boletos  # ← IMPORTA O MÓDULO CORRETO
                    ...
                    if dados_boleto:
                        ...
                        print(f"✅ [DATAFRAME] Boleto adicionado à tabela")
                except Exception as e:
                    print(f"⚠️ [DATAFRAME] Erro ao processar: {e}")

            except ValueError as e:  # ← CAPTURA ERROS DE NEGÓCIO
                print(f"⚠️ [ERRO CAPTURADO NO LOOP] {e}")
    ...
    def _loop_apartamentos(self, apto_texto: str,
        ...
        total = loc.count()
        print(f"[INFO] Encontrados {total} itens para '{apto_texto}'.")
    ...
    def _loop_apartamentos_dinamico(self, item_spec_links: dict, item_spec_buttons: dict, identificador_base: str,
        ...
        try:
            ...
            print(f"⚠️  [MENSAGEM DE ERRO DETECTADA] {texto_erro}")
            ...
            print(f"⚠️  Erro ao verificar mensagem: {type(e).__name__}")
            pass

        # ===== 1. TENTA LINKS PRIMEIRO ===== ↓
        try:
            print(f"🔍 [BUSCANDO] Procurando por links de boletos...")
            ...
            if total_links > 0:
                print(f"✅ [RESULTADO] Encontrados {total_links} link(s)")
                for i in range(total_links):
                    try:
                        ...
                        try:
                            ...
                            if dados: self.boletos_processados.append(dados)
                        except:
                            pass

                    except Exception as e:
                        print(f"⚠️ Erro ao baixar link {i + 1}: {e}")
                return  # Sucesso - sai da função

        except Exception:
            print(f"ℹ️  [INFO] Links não encontrados - tentando botões...")

        # ===== 2. TENTA BOTÕES SE NÃO ACHOU LINKS ===== ↓
        try:
            print(f"🔍 [BUSCANDO] Procurando por botões 'GERAR BOLETO'...")
            ...
            if total_buttons > 0:
                print(f"✅ [RESULTADO] Encontrados {total_buttons} botão(ões)")
                for i in range(total_buttons):
                    try:
                        ...
                        print(f"⚠️ Erro ao baixar botão {i + 1}: {e}")
                return  # Sucesso - sai da função
    ...
    def _download_recibos_icondo(self, partial_text: str, identificador: str,
        ...
        try:
            print(f"[download_recibos_icondo] Procurando links '{partial_text}'...")
            ...
            print(f"[INFO] {total_encontrados} link(s) '{partial_text}' encontrado(s)")

            if total_encontrados > max_boletos:
                print(f"⚠️ Limitando a {max_boletos} boletos")
            ...
            for i in range(len(links)):
                try:
                    ...
                    if i >= len(current_links):
                        print(f"⚠️ Link {i + 1} não existe mais, parando")
                        break

                    link = current_links[i]

                    print(f"Baixando boleto {i + 1}/{len(links)}...")
                    ...
                    if caminho_destino.exists():
                        baixados += 1
                        print(f"✓ Boleto {i + 1} salvo: {nome_arquivo}")
                        ...
                        if hasattr(self, 'header_fn') and self.header_fn:
                            try:
                                ...
                                print(f"⚠️ Erro ao adicionar cabeçalho PDF: {e}")
                    else:
                        falhas += 1
                        print(f"✗ Falha ao salvar boleto {i + 1}")
                    ...
                    falhas += 1
                    print(f"✗ Erro ao baixar boleto {i + 1}: {e}")
                    continue
    ...
    def _loop_apartamentos_popup(self, item_spec, identificador_base):
        ...
        """
        print(f"🔍 [POPUP MODE] Procurando boletos...")
        ...
        if total == 0:
            print("⚠️ Nenhum boleto encontrado para clicar.")
            return

        print(f"✅ Encontrados {total} boleto(s).")
        ...
        for i in range(total):
            try:
                ...
                print(f"   🔄 Processando boleto {i + 1}...")
                ...
                print(f"   📥 Salvo como PDF: {nome_arq}")
                ...
                try:
                    ...
                    print(f"   ⚠️ Erro processamento pós-download: {e_proc}")

            except Exception as e:
                print(f"   ❌ Erro ao processar boleto {i + 1}: {e}")
    ...
    def _run_step(self, step: dict, context: dict):
        ...
        if action == "goto":
            ...
            try:
                ...
                print(f"⚠️  [PARAR_SE_VISIVEL] Mensagem capturada:\n{mensagem_final}")
                ...
                print(f"⚠️  [PARAR_SE_VISIVEL] Exceção ignorada: {type(e).__name__}: {e}")
                pass
            ...
            try:
                ...
                try:
                    ...
                    print(f"✅ [PARAR_SE_NAO_VISIVEL] Elemento encontrado, continuando...")
                    ...
                    print(f"⚠️  [PARAR_SE_NAO_VISIVEL] Elemento não encontrado:\n{mensagem_final}")
                    raise ValueError(mensagem_final)
                ...
                print(f"⚠️  [PARAR_SE_NAO_VISIVEL] Exceção: {type(e).__name__}: {e}")
                raise
            ...
            for idx, loc_spec in enumerate(locators):
                try:
                    ...
                    loc.click()
                    print(f"✅ [CLICK MULTI] Clicou usando seletor {idx + 1}/{len(locators)}")
                    sucesso = True
                    break  # ← Para no primeiro que funcionar
                except Exception as e:
                    print(f"⚠️ [CLICK MULTI] Seletor {idx + 1} falhou: {e}")
                    continue
            ...
            try:
                ...
                print(f"✅ [CLIQUE] Elemento clicado com force=True")

            except Exception as e:
                print(f"ℹ️  [CLIQUE] Elemento não disponível - pulando...")
            ...
            try:
                ...
                print(f"🔍 [TRY FUZZY] Procurando items_css='{items_css}'")
                if text_selector:
                    print(f"   text_selector='{text_selector}'")
                if click_selector:
                    print(f"   click_selector='{click_selector}'")
                if alvo_apto:
                    print(f"   alvo_apartamento='{alvo_apto}'")
                ...
                if text_selector:
                    print(f"🔍 [TRY FUZZY] Executando JavaScript para extrair textos...")
                ...
                for i, t in enumerate(textos):
                    ...
                    if alvo_apto_norm:
                        if alvo_norm in t_norm and alvo_apto_norm in t_norm:
                            best_i = i
                            best_txt = t
                ...
                if best_i < 0:
                    for i, t in enumerate(textos):
                        ...
                        score = w_score if w_score >= cutoff else max(w_score, p_score)

                        # Bonus se achar apartamento
                        if alvo_apto_norm and alvo_apto_norm in tn:
                ...
                if click_selector:
                    ...
                    print(f"  ✅ Clicou no card correto (via {click_selector})")
                else:
                    elemento.click(timeout=5000)
                    print(f"  ✅ Clicou no card correto")

                self.condo_selecionado_site = f"{best_titulo} - {best_apto}"
                print(f"✅ [CARD SELECIONADO]")

            except Exception as e:
                print(f"⚠️ [TRY FUZZY] ERRO: {e}")
                print(f"ℹ️  [SELEÇÃO OPCIONAL] Não encontrou lista de condomínios")
                print(f"   → Continuando sem selecionar...")


            except Exception as e:
                print(f"⚠️ [TRY FUZZY] ERRO: {e}")
                print(f"ℹ️  [SELEÇÃO OPCIONAL] Não encontrou lista")
                print(f"   → Continuando...")
            ...
            try:
                ...
                if not self.context_linha:
                    print("⚠️  [LIVEFACILITIES] Contexto não disponível, pulando verificação")
                else:
                    ...
                    try:
                        ...
                        print("ℹ️  [LIVEFACILITIES] Detectado login ativo, verificando usuário...")
                        ...
                        try:
                            ...
                            input_login = self.page.locator('input[name*="login" i], input[placeholder*="login" i]').first
                        ...
                        if usuario_logado and usuario_esperado and usuario_esperado == usuario_logado:
                            print(f"✅ [LIVEFACILITIES] Já logado com usuário correto: {usuario_logado}")
                            print("⏭️  [LIVEFACILITIES] Pulando etapa de login...")
                        ...
                        print(f"🚪 [LIVEFACILITIES] Fazendo logout (usuário: {usuario_logado or 'não identificado'} ≠ {usuario_esperado})...")
                        link_sair.click()

                        # Aguarda voltar para tela de login
                        self.page.wait_for_selector('input[type="text"][name*="login"], input[type="text"][placeholder*="login" i]', timeout=5000)
                        print("✅ [LIVEFACILITIES] Logout realizado com sucesso")

                    except:
                        # Não está logado, tudo ok
                        print("ℹ️  [LIVEFACILITIES] Não há login ativo, prosseguindo...")
                        pass

            except Exception as e:
                print(f"⚠️  [LIVEFACILITIES] Erro ao verificar login: {e}")
            ...
            try:
                ...
                logged_in_selector = self.page.locator("a[href*='condominio'], .navbar, #menu-principal, .condominio-card")

                # Aguarda um pouco para ver se está logado
                logged_in_selector.first.wait_for(state="visible", timeout=3000)

                # Se chegou aqui, está logado
                print(f"   ✅ [SUPERLOGICA] Já está logado - pulando login")
                ...
                print(f"   🔓 [SUPERLOGICA] Não está logado - fazendo login")
                pass
            ...
            if tem_erro:
                print(f"⚠️ Erro detectado mas continuando (stop_if_error=False)")
            ...
            try:
                ...
                if len(tabelas) == 1:
                    print(f"ℹ️  [LIVEFACILITIES] Apenas 1 unidade - pulando seleção")
                    return
                ...
                # print(f"🔍 [LIVEFACILITIES] {len(tabelas)} unidades encontradas")
                #
                ...
                for i, tabela in enumerate(tabelas):
                    try:
                        ...
                        # print(f"   [{i}] {condo_completo} - {bloco} {unidade}")

                    except Exception as e:
                        print(f"⚠️ Erro ao extrair tabela {i}: {e}")
                        continue
                ...
                print(f"   📋 Buscando: '{alvo_condo}' + '{alvo_apto}'")
                print(f"   🔍 Normalizado: '{alvo_norm}'\n")
                ...
                # Fuzzy
                if best_i < 0:
                    print(f"   🔍 Calculando scores fuzzy...")
                    for opt in opcoes:
                        ...
                        if score >= 0.50:
                            print(f"      [{opt['idx']}] score={score:.2f} (W:{w_score:.2f}, P:{p_score:.2f}) - {opt['condo_completo'][:50]} - {opt['bloco']} {opt['unidade']}")
                ...
                lines.append(f"                       🌐 Encontrado no SITE: {best_opt['condo_completo']} - {best_opt['bloco']} {best_opt['unidade']}")
                ...
                if best_score < cutoff:
                    ...
                    print(f"   Melhor score encontrado: {best_score:.2f}")
                    print(f"   Opções disponíveis:")
                    for opt in opcoes[:10]:
                        print(f"   - {opt['condo_completo']} - {opt['bloco']} {opt['unidade']}")
                    if len(opcoes) > 10:
                        print(f"   ... e mais {len(opcoes) - 10}")
                    # ===== FIM ===== ↑
                    raise ValueError(f"❌ Unidade não encontrada! Melhor score: {best_score:.2f} < cutoff {cutoff:.2f}")

                # Clica no link
                best_opt["link"].click()
                print(f"✅ [LIVEFACILITIES] Unidade selecionada")
                ...
                print(f"⚠️ [LIVEFACILITIES UNIDADE] ERRO: {e}")
                raise
            ...
            try:
                ...
                timeout = 60000
                print("🚀 [LIVEFACILITIES] Iniciando verificação e download de boletos...")
                ...
                if loc_erro.is_visible() and loc_erro.inner_text().strip() != "":
                    ...
                    texto_erro = texto_erro.replace('\n', ' ').replace('\r', ' ')
                    texto_erro = re.sub(r' +', ' ', texto_erro).strip()
                    print(f"⚠️ Mensagem de erro detectada na página: {texto_erro}")
                ...
                for i in range(qtd_boletos):
                    ...
                    try:
                        ...
                        print("   ⏳ Aguardando botão de download no modal...")
                        # Dispara download
                        with self.page.expect_download(timeout=timeout) as download_info:
                        ...
                        print(f"   📥 Download salvo: {nome_arquivo}")
                        # ===== SEU BLOCO CORRIGIDO AQUI ===== ↓

                        # 1. Adiciona Cabeçalho
                        try:
                            # aux.adicionarcabecalhopdf_topo_adaptativo espera string
                            aux.adicionarcabecalhopdf_topo_adaptativo(str(destino), str(destino), codigo_cliente, senha_base=self.cpf_senha)
                            print(f"   ✅ [CABEÇALHO] Código {codigo_cliente} adicionado")
                        except Exception as e:
                            print(f"   ⚠️ [CABEÇALHO] Erro: {e}")
                        ...
                        try:
                            ...
                            if dados_boleto:
                                self.boletos_processados.append(dados_boleto)
                                print(f"   ✅ [SUCESSO] Classificado e movido!")
                                baixados += 1
                            else:
                                print(f"   ⚠️ Arquivo baixado, mas não classificado (mantido na raiz).")
                                baixados += 1  # Conta como baixado pois o arquivo existe

                        except Exception as e_class:
                            print(f"   ❌ Erro ao processar/mover boleto: {e_class}")
                            baixados += 1

                        # ===== FIM DO BLOCO ===== ↑

                    except Exception as e_inner:
                        print(f"   ❌ Erro ao baixar boleto {i + 1}: {e_inner}")

                    finally:
                        # --- RESET: REFRESH NA PÁGINA ---
                        # Essencial para fechar o modal e limpar o DOM
                        print("   🔄 Atualizando página para fechar modal...")
                        self.page.reload()
                ...
                print(f"❌ [ERRO CRÍTICO] Falha no fluxo LiveFacilities: {e}")
                raise
            ...
            try:
                ...
                for i, loc_spec in enumerate(locators_list):
                    try:
                        ...
                        loc.fill(valor)
                        print(f"✅ [FILL MULTI] Preencheu usando seletor {i + 1}/{len(locators_list)}")
                        sucesso = True
                        break
                    except Exception as e:
                        print(f"⚠️ [FILL MULTI] Seletor {i + 1} falhou: {e}")
                        continue
            ...
            print(f"🔍 [CIPA] Procurando boletos para download...")
            ...
            try:
                ...
                try:
                    ...
                    print(f"⚠️ [CIPA] Nenhum ícone de download encontrado")
                    return

                total = loc.count()
                print(f"✅ [CIPA] Encontrados {total} boleto(s) para download")
                ...
                for i in range(total):
                    try:
                        ...
                        if loc.count() <= i:
                            print(f"⚠️ [CIPA] Boleto {i + 1} não existe mais")
                            break
                        ...
                        print(f"   🔄 Baixando boleto {i + 1}/{total}...")
                        ...
                        try:
                            ...
                            if dados_boleto:
                                self.boletos_processados.append(dados_boleto)
                                print(f"   ✅ [DATAFRAME] Boleto adicionado")
                        except Exception as e:
                            print(f"   ⚠️ [DATAFRAME] Erro: {e}")
                ...
                print(f"❌ [CIPA] Erro no loop de download: {e}")
                raise
            ...
            print(f"🔍 [CIPA SESSÃO] Verificando sessão ativa...")
            print(f"   📋 Esperado: {usuario_esperado} | {condo_esperado}")
            try:
                ...
                try:
                    ...
                    print(f"   ⚪ Não está logado - seguindo para login")
                    return
                ...
                try:
                    ...
                    if el_condo.is_visible(timeout=2000):
                        ...
                        print(f"   🏢 Condomínio logado: {condo_logado}")
                except:
                    pass
                ...
                print(f"   ⚠️ Erro: {e} - continuando normalmente")
                return
            ...
            """
            print("📋 [CIPA] Capturando lista de unidades...")

            try:
                # Clica em "Unidades"
                link_unidades = self.page.get_by_role("link", name=re.compile(r"Unidades", re.I))
                ...
                # Captura spans
                unidades_elements = self.page.locator("span.text-default.font-bold.text-md").all()

                unidades = []
                for el in unidades_elements:
                    texto = el.inner_text().strip()
                    if texto and ("BL" in texto or "APT" in texto):
                ...
                print(f"   ✅ {len(unidades_unicas)} unidades capturadas:")
                for u in unidades_unicas[:10]:
                    print(f"      • {u}")
                if len(unidades_unicas) > 10:
                    print(f"      ... e mais {len(unidades_unicas) - 10}")
            ...
            """
            Versão OTIMIZADA: Valida unidade contra cache e DIGITA no autocomplete.
            """
            apartamento_alvo = step.get("apartamento", "")
            print(f"🔍 [CIPA FILTRO] Configurando unidade: {apartamento_alvo}")

            try:
                # ===== PRÉ-VALIDAÇÃO COM CACHE ===== ↓
                unidade_para_digitar = apartamento_alvo  # Default

                if hasattr(self, 'cipa_unidades_cache') and self.cipa_unidades_cache:
                    print(f"   📋 Validando contra {len(self.cipa_unidades_cache)} unidades...")
                    ...
                    try:
                        ...
                        # Pega o texto EXATO da lista para digitar
                        unidade_para_digitar = result['best_txt']
                        print(f"   ✅ Match: '{unidade_para_digitar}'")

                    except ValueError as e:
                        raise ValueError(f"❌ Unidade '{apartamento_alvo}' NÃO EXISTE!\n   {e}")
                else:
                    print(f"   ⚠️ Sem cache - usando valor original")
                # ===== FIM PRÉ-VALIDAÇÃO ===== ↑

                # ===== 1. ABRE MODAL ===== ↓
                print("   🖱️ Abrindo modal de filtros...")
                btn_abrir = self.page.get_by_role("button", name=re.compile(r"Filtros", re.I))
                btn_abrir.wait_for(state="visible", timeout=10000)
                btn_abrir.click()
                ...
                # ===== 2. DIGITA NO AUTOCOMPLETE ===== ↓
                print(f"   ⌨️ Digitando '{unidade_para_digitar}'...")

                # Localiza o input de Unidade
                input_unidade = self.page.locator("mat-form-field").filter(
                    has_text="Unidade"
                ).locator("input").first

                input_unidade.wait_for(state="visible", timeout=5000)
                ...
                # DIGITA o texto validado
                input_unidade.fill(unidade_para_digitar)
                self.page.wait_for_timeout(1500)  # Aguarda autocomplete filtrar

                print(f"   ✅ Digitado!")
                # ===== FIM ===== ↑

                # ===== 3. SELECIONA A OPÇÃO FILTRADA ===== ↓
                print("   🔍 Selecionando opção filtrada...")

                # Aguarda opções aparecerem
                try:
                    self.page.locator(".mat-option-text").first.wait_for(state="visible", timeout=5000)
                except:
                    raise ValueError(f"Nenhuma opção apareceu após digitar '{unidade_para_digitar}'")

                opcoes = self.page.locator(".mat-option-text").all()

                if len(opcoes) == 0:
                    raise ValueError(f"Autocomplete não retornou opções para '{unidade_para_digitar}'")

                elif len(opcoes) == 1:
                    # Apenas 1 opção - seleciona direto
                    texto = opcoes[0].inner_text().strip()
                    print(f"   ✅ Opção única: '{texto}'")
                    opcoes[0].click()

                else:
                    # Múltiplas opções - pega a primeira (autocomplete já filtrou)
                    texto = opcoes[0].inner_text().strip()
                    print(f"   ⚠️ {len(opcoes)} opções - selecionando primeira: '{texto}'")
                    opcoes[0].click()

                self.page.wait_for_timeout(1000)
                # ===== FIM ===== ↑

                # ===== 4. SELECIONA STATUS "EM ABERTO" ===== ↓
                print("   🎯 Selecionando Status 'Em Aberto'...")

                campo_status = self.page.locator(
                    "xpath=/html/body/div[2]/div[2]/div/mat-dialog-container/boleto-filter-dialog/div/div[2]/mat-form-field[2]/div/div[1]/div"
                )

                self.page.evaluate("el => el.click()", campo_status.element_handle())
                self.page.wait_for_timeout(2000)

                opcoes_status = self.page.locator(".mat-option-text").all()

                for status in opcoes_status:
                    if "Em Aberto" in status.inner_text():
                        self.page.evaluate("el => el.click()", status.element_handle())
                        self.page.wait_for_timeout(500)
                        print("   ✅ Status selecionado")
                        break
                # ===== FIM ===== ↑
                ...
                btn_filtrar = self.page.locator(
                    "xpath=/html/body/div[2]/div[2]/div/mat-dialog-container/boleto-filter-dialog/div/div[3]/button[2]/span[1]"
                )

                self.page.evaluate("el => el.click()", btn_filtrar.element_handle())
        ...
        try:
            ...
            if menu_usuario.is_visible(timeout=3000):
                ...
                if btn_sair.is_visible(timeout=2000):
                    btn_sair.click()
                    self.page.wait_for_load_state("networkidle", timeout=10000)
                    print(f"   ✅ Logout completo realizado")
                    return

            # Fallback: limpa cookies
            print(f"   🛡️ Fallback: limpando cookies")
            self.page.context.clear_cookies()
    ...
    def _voltar_selecao_condo_cipa(self):
        ...
        try:
            ...
            if menu_condo.is_visible(timeout=3000):
                print(f"   🏢 Clicando no menu do condomínio...")
                menu_condo.click()
                self.page.wait_for_timeout(1000)
                ...
                if btn_sair.is_visible(timeout=2000):
                    btn_sair.click()
                    self.page.wait_for_timeout(2000)
                    print(f"   ✅ Voltou para seleção de condomínios")
                    return True

            # Fallback: navega direto
            print(f"   🔄 Fallback: navegando para lista de condomínios")
            self.page.goto("https://cipafacil.digital/condominiums")
            self.page.wait_for_load_state("networkidle")
            return True

        except Exception as e:
            print(f"   ⚠️ Erro ao voltar para seleção: {e}")
            return False
...
LOGINS = {
    ...
    "webware": [
        ...
        {"scope": "login", "action": "fill",
         "locator": {"by": "css", "selector": "input[name='mem'], input[name='usuario'], input[name='login'], input[id='usuario']"},
         "value": "${USUARIO}"},
    ...
    "imodata": [
        ...
        # 8. Verifica erro
        {"scope": "login", "action": "verificar_erro_login",
         "locator": {"by": "xpath", "selector": "//div[contains(@style, 'color:red')] | //div[contains(text(), 'não conferem')]"},
         "timeout": 3000,
         "stop_if_error": True}
    ],
    ...
    "cipa": [
        ...
        # Aguarda área logada
        {"scope": "login", "action": "esperar_selector",
         "locator": {"by": "css", "selector": "condominium-list, div.filter-article, a[href*='boleto']"},
         "timeout": 20000},
    ],
    ...
    "nacional": [
        ...
        {"scope": "login", "action": "verificar_erro_login",
         "locator": {"by": "xpath", "selector": "//*[contains(text(), 'desabilitado') or contains(text(), 'Lamento') or contains(@class, 'erro') or contains(@class, 'mensagem-erro')]"},
         "timeout": 3000,
         "stop_if_error": True},
    ],
...
ADMIN_FLOWS = {
    ...
    "livefacilities": [
        ...
        # 4. Menu FINANCEIRO (EXATAMENTE como no codegen!)
        {
            "action": "click",
            "locator": {"by": "role", "role": "link", "name": " FINANCEIRO - +"},
            "timeout": 10000
        },

        # 5. Submenu 2ª VIA DE BOLETO (EXATAMENTE como no codegen!)
        {
            "action": "click",
            "locator": {"by": "role", "role": "link", "name": "ª VIA DE BOLETO"},
            "timeout": 10000
        },
    ...
    "nacional": [
        ...
        {
            "action": "try_click",
            "locator": {"by": "css", "selector": "a:has-text('2ª Via'), a:has-text('Boleto'), a:has-text('Segunda Via')"},
            "timeout": 5000
        },
    ...
    "cipa": [
        # PASSO 1: Esperar o card do condomínio aparecer (O que resolveu sua seleção)
        {
            "action": "esperar_selector",
        ...
        # PASSO 2: Selecionar condomínio (Fuzzy original)
        {
            "action": "try_fuzzy_lista_fechada",
            "value": "${CONDOMINIO}",
        ...
        # PASSO 3: MUITO IMPORTANTE - Esperar a troca de página para o Dashboard
        {
            "action": "esperar_url",
            "pattern": "/dashboard",
            "timeout": 15000
        ...
        # PASSO 4: Clicar no link de Boletos
        {
            "action": "click",
        ...
        # PASSO 5: Rodar a função de filtro que acabamos de criar acima
        {
            "action": "filtro_cipa",
            "apartamento": "${APARTAMENTO}"
        },

        # PASSO 6: Verificar se deu "Vazio" (Seletor de imagem do seu Selenium)
        {
            "action": "parar_se_visivel",
    ...
    # ===== ADMINISTRADORA NACIONAL =====
    "nacional": [
        # Sem seleção de condomínio - já carrega direto na página
        # Apenas navega para a tela de boletos/2ª via
    ],
}
...
FLOW_CONFIG = {
    ...
    # ===== iCondo LOGIN + Superlógica FLOW ===== ↓
    "HILMAR EMPREENDIMENTOS IMOBILIARIOS": ("icondo", "superlogica"),
    "ESTASA": ("icondo", "superlogica"),
    "ADIPLANTEC": ("icondo", "superlogica"),

    # ===== iCondo COMPLETO (login icondo + flow icondo) ===== ↓
    "A.G. RIO IMÓVEIS": ("icondo", "icondo"),
    "ADMINISTRADORA REALCAD": ("icondo", "icondo"),
    "CENTRIMÓVEIS": ("icondo", "icondo"),
    "CINOCRED IMÓVEIS": ("icondo", "icondo"),
    "CNI-AJF COSTA NETO CONDOMÍNIOS E LO": ("icondo", "icondo"),
    "IRIGON": ("icondo", "icondo"),
    "J.B.ANDRADE IMÓVEIS": ("icondo", "icondo"),
    "JC CONSULTORIA": ("icondo", "icondo"),
    "LUMEN ADMINISTRADORA": ("icondo", "icondo"),
    "ML ADMINISTRAÇÃO DE IMÓVEIS": ("icondo", "icondo"),
    "NOTA 10 X": ("icondo", "icondo"),
    "PERSONAL PRIME ADMINISTRADORA": ("icondo", "icondo"),
    "PRECISÃO ADMINISTRADORA DE IMÓVEIS": ("icondo", "icondo"),
    "PRED RIO ADMINISTRAÇÃO IMOBILIÁRIA": ("icondo", "icondo"),
    "PROHOME ADM E CONSULTORIA IMÓVEIS": ("icondo", "icondo"),
    "PROTEST": ("icondo", "icondo"),
    "QUALITY HOUSE": ("icondo", "icondo"),
    "ZIRTAEB": ("icondo", "icondo"),
    ...
    # ===== FERNANDO E FERNANDES =====
    "FERNANDO E FERNANDES": ("fernandoefernandes", "fernandoefernandes"),
...
def main():
    ...
    banco = aux.Banco(r"C:\Users\efart\OneDrive\Área de Trabalho\Scai_local.WMB")
    ...
    with sync_playwright() as p:
        ...
        for idx_linha, linha in enumerate(linhas):
            ...
            if not usr_check or not pwd_check:
                ...
                print(f"❌ [{codigo_check}] {msg_erro} | Pulando...")
            ...
            print(f"🔍 [DEBUG] family_now={family_now}, prev_creds={prev_creds is not None}, on_login_page={on_login_page}, ja_logado={ja_logado}")
            ...
            if "cipafacil" in family_now:
                ...
                # ===== DECISÃO CIPA ===== ↓
                if same_creds and same_condo_cipa:
                    # Caso 1: Usuário E condomínio iguais → NÃO FAZ NADA
                    print(f"♻️  [CIPA] Usuário e condomínio iguais - pulando logout/navegação")
                    need_login = False

                elif same_creds and not same_condo_cipa:
                    # Caso 2: Usuário igual, condomínio diferente → SÓ SAI DO CONDOMÍNIO
                    print(f"♻️  [CIPA] Mesmo usuário, condomínio diferente - voltando pra área de seleção")
                    ...
                    if not on_login_page and not esta_em_blank:
                        try:
                            ...
                            if menu_condo.is_visible(timeout=2000):
                                print(f"   🏢 Saindo do condomínio atual...")
                                menu_condo.click(timeout=5000)
                                page.wait_for_timeout(1000)

                                page.get_by_role("menuitem", name="Sair").click(timeout=5000)
                                page.wait_for_timeout(1500)
                                print(f"   ✅ Voltou para área de seleção de condomínios")
                            else:
                                print(f"   ℹ️  Já está na área de seleção")

                        except Exception as e:
                            print(f"   ⚠️ Erro ao sair do condomínio: {e}")
                    ...
                    if not on_login_page and not esta_em_blank:
                        print(f"🔄 [LOGOUT] Usuário diferente - fazendo logout completo...")
                        try:
                            efetuar_logout(page, target_site)
                        except Exception as e:
                            print(f"⚠️ [LOGOUT] Erro: {e}")
                # ===== FIM DECISÃO CIPA ===== ↑
            ...
            # ===== LÓGICA PADRÃO PARA OUTRAS PLATAFORMAS ===== ↓
                ...
                # ===== LIVEFACILITIES: FORÇA LOGOUT NA PRIMEIRA EXECUÇÃO ===== ↓
                # Força logout se for primeira linha (prev_creds = None) e LiveFacilities
                # Isso garante estado limpo mesmo que haja sessão de execução anterior
                skip_logout = False
                if prev_creds is None and family_now == "livefacilities":
                    print(f"🔄 [LIVEFACILITIES] Primeira execução - verificando estado...")
                    # print(f"   on_login_page={on_login_page}, esta_em_blank={esta_em_blank}, ja_logado={ja_logado}")

                    if not on_login_page and not esta_em_blank:
                        print(f"   → Forçando logout para estado limpo...")
                        need_login = True
                        skip_logout = False  # FORÇA logout
                    else:
                        print(f"   → Já está em tela de login ou blank, não precisa logout")
                # ===== FIM ===== ↑
                ...
                if need_login and proximo_tem_mesmo_usuario and proximo_tem_mesmo_host and proximo_tem_mesma_family:
                    ...
                    if not on_login_page and prev_creds is not None:
                        # Não está na tela de login E já logou antes → REUSA a sessão
                ...
                if need_login and not on_login_page and not esta_em_blank and not skip_logout:
                    print(f"🔄 [LOGOUT] Fazendo logout antes de novo login...")
                    try:
                        efetuar_logout(page, target_site)
                    except Exception as e:
                        print(f"⚠️ [LOGOUT] Erro: {e}")
                elif need_login and on_login_page:
                    print(f"ℹ️  [LOGIN] Já está na tela de login - pulando logout")
                elif need_login and esta_em_blank:
                    print(f"ℹ️  [INÍCIO] Browser em about:blank - pulando logout")
                elif not need_login:
                    print(f"♻️  [REUSO] Mantendo login existente - pulando login/logout")
            # ===== FIM LÓGICA PADRÃO ===== ↑
            ...
            if need_login and "livefacilities" in (site_family or "").lower():
                print(f"   🛡️ [STARTUP] Forçando limpeza de sessão (garantia de Logoff)...")
                try:
                    ...
                    if target_url:
                        print(f"       ↪️ Recarregando página de login: {target_url}")
            ...
            if (prev_condo_base is not None and
                ...
                if _norm(prev_condo_base) == condo_alvo_norm:
                    ...
                    reuso_w_score = 1.0
                    reuso_p_score = 1.0
                    print(f"♻️  [OTIMIZAÇÃO] Comparação direta: '{condo_alvo}' == '{prev_condo_base}'")
                ...
                # 2. Tenta fuzzy match (robusto!)
                    ...
                    if fuzzy_score >= 0.80:  # Mesmo cutoff do fuzzy_lista_fechada
                        ...
                        print(f"♻️  [OTIMIZAÇÃO] Fuzzy match: '{condo_alvo}' ~= '{prev_condo_site}' (score={fuzzy_score:.2f})")

            if same_condo:
                # ===== MENSAGEM ESPECÍFICA PARA iCondo+Superlógica ===== ↓
                if "icondo" in login_type.lower() and eh_flow_superlogica:
                    print(f"   → Pulando seleção/expansão de condomínio (iCondo + flow Superlógica)")
                else:
                    print(f"   → Pulando seleção/expansão de condomínio")
                # ===== FIM ===== ↑
                ...
                lines.append(f"                     📊 WRatio: {reuso_w_score:.2f} | partial_ratio: {reuso_p_score:.2f}")
            ...
            print(f"\n{'=' * 70}")
            print(f"🔹 PROCESSANDO [{codigo}] {cond} - {apto}")
            ...
            try:
                ...
                print(f"⚠️ Erro ao gravar NOMEFUNCAO: {e}")
            ...
            try:
                ...
                print(f"⚠️ Erro ao gravar MATCH_LOG: {e}")
            ...
            try:
                ...
                print(f"⚠️ Erro ao gravar PROBLEMALOGIN: {e}")
            ...
            try:
                ...
                print(f"⚠️ Erro ao gravar ERROS: {e}")
            # ===== FIM ===== ↑
            ...
            if proximo_tem_mesmo_usuario and proximo_tem_mesmo_host and proximo_tem_mesma_family:
                ...
                # print(f"   ✅ [BASELINE] Atualizando (próximo é igual)")
            else:
                # Próximo é diferente → verifica se está logado
                if esta_logado(page, family_now, timeout_ms=900):
                    atualiza_baseline = True
                    # print(f"   ✅ [BASELINE] Atualizando (verificou sessão)")
                # else:
                    # print(f"   ⚠️  [BASELINE] NÃO atualizando (sessão inválida)")
            ...
            if atualiza_baseline:
                ...
                if st["ok"] or condo_foi_selecionado:
                    ...
                    if not st["ok"] and condo_foi_selecionado:
                        print(f"   ⚠️  [BASELINE] Condomínio salvo apesar do erro (erro foi no apartamento)")
            ...
            if not st["ok"]:
                ...
                if proximo_tem_mesmo_usuario and proximo_tem_mesmo_host and proximo_tem_mesma_family:
                    ...
                    # print(f"ℹ️  [OTIMIZAÇÃO] Próximo registro é igual - assumindo sessão válida")
                else:
                ...
                if login_falhou or sessao_invalida:
                    print(f"🔄 [ERRO DE LOGIN/SESSÃO] Resetando baseline para forçar login na próxima linha.")
                    ...
                    print(f"ℹ️  [ERRO DE NEGÓCIO] Sessão válida - mantendo login para próxima linha.")
            # ====================================================================
        ...
        if executor.boletos_processados:
            print("\n" + "=" * 60)
            print("SALVANDO DATAFRAME DE BOLETOS...")
            print("=" * 60)
            ...
            try:
                ...
                print("✅ Formatação Excel aplicada!")
        ...
        try:
            ...
            print(f"⚠️ Erro ao formatar Excel (não crítico): {e}")
