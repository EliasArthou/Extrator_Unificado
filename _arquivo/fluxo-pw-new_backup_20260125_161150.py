...
# ===================== PROGRESS LOGGER COM BARRA FIXA NO TOPO =====================
class FixedProgressLogger:
    """
    Barra de progresso FIXA NO TOPO do terminal.
    Logs rolam normalmente abaixo com scroll nativo.
    """
    ...
    def __init__(self, total: int, description: str = "Processando..."):
        ...
        self.progress = Progress(
            ...
            TextColumn("[bold cyan]{task.completed}[/bold cyan]/[cyan]{task.total}[/cyan]"),
            TaskProgressColumn(),
            TextColumn("│"),
            TextColumn("Restante:"),
            TimeRemainingColumn(),
        ...
        # Altura da barra (3 linhas: borda superior, conteúdo, borda inferior)
        self._bar_height = 3

    def _render_bar(self) -> str:
        """Renderiza a barra como string."""
    ...
    def _update_bar(self):
        """Atualiza a barra no topo do terminal."""
    ...
    def _background_updater(self):
        """Thread que atualiza a barra periodicamente."""
        while self._running:
            self._update_bar()
            time.sleep(0.25)
    ...
    def start(self):
        """Inicia o display."""
        # Limpa tela e configura região de scroll
        print("\033[2J", end="", flush=True)  # Limpa tela
        print("\033[H", end="", flush=True)   # Move para o topo

        # Imprime a barra inicial
        bar_text = self._render_bar()
        print(bar_text, end="", flush=True)

        # Define região de scroll (abaixo da barra)
        # \033[{top};{bottom}r define a região de scroll
        terminal_height = console.height or 24
        print(f"\033[{self._bar_height + 1};{terminal_height}r", end="", flush=True)

        # Move cursor para abaixo da barra
        print(f"\033[{self._bar_height + 1};1H", end="", flush=True)

        # Inicia thread de atualização
        self._running = True
        self._update_thread = threading.Thread(target=self._background_updater, daemon=True)
    ...
    def stop(self):
        ...
        # Restaura região de scroll normal
        print("\033[r", end="", flush=True)

    def log(self, msg: str):
        """Imprime log na área de scroll (abaixo da barra)."""
        with self.lock:
            console.print(msg)
            ...
            try:
                ...
                if skip_mode == "SKIP_TO_BOLETOS":
                    ...
                    if action not in ["click"] or "boleto" not in str(passo.get("locator", {})).lower():
                        log(f"[SKIP] Passo {i + 1} - indo direto para boletos")
                        continue
                    else:
    ...
    def _run_step(self, step: dict, context: dict):
        ...
        if action == "goto":
            ...
            try:
                ...
                if len(tabelas) == 1:
                    log(f"ℹ️  [LIVEFACILITIES] Apenas 1 unidade - pulando seleção")
                    return
            ...
            try:
                ...
                if not usuario_igual:
                    ...
                    log(f"   ✅ Sessão REUTILIZADA - direto para boletos")
            ...
            """
            log("📋 [CIPA] Capturando lista de unidades...")
            ...
            try:
                ...
                if len(unidades_unicas) > 10:
                    log(f"      ... e mais {len(unidades_unicas) - 10}")
            ...
            log(f"🔍 [CIPA FILTRO] Configurando unidade: {apartamento_alvo}")
            ...
            try:
                ...
                if hasattr(self, 'cipa_unidades_cache') and self.cipa_unidades_cache:
                    log(f"   📋 Validando contra {len(self.cipa_unidades_cache)} unidades...")
                    ...
                    try:
                        ...
                        log(f"   ✅ Match: '{unidade_para_digitar}'")

                    except ValueError as e:
                        raise ValueError(f"❌ Unidade '{apartamento_alvo}' NÃO EXISTE!\n   {e}")
                else:
                    log(f"   ⚠️ Sem cache - usando valor original")
                # ===== FIM PRÉ-VALIDAÇÃO ===== ↑
                ...
                log(f"   ⌨️ Digitando '{unidade_para_digitar}'...")
                ...
                log("   🔍 Selecionando opção filtrada...")
                ...
                if len(opcoes) == 0:
                    ...
                    log(f"   ✅ Opção única: '{texto}'")
                    opcoes[0].click()

                else:
                    # Múltiplas opções - pega a primeira (autocomplete já filtrou)
                    texto = opcoes[0].inner_text().strip()
                    log(f"   ⚠️ {len(opcoes)} opções - selecionando primeira: '{texto}'")
                    opcoes[0].click()
            ...
            for idx_linha, linha in enumerate(linhas):
                ...
                if "cipafacil" in family_now:
                    ...
                    if same_creds and same_condo_cipa:
                        ...
                        log(f"♻️  [CIPA] Mesmo usuário, condomínio diferente - voltando pra área de seleção")
                        ...
                        if not on_login_page and not esta_em_blank:
                            try:
                                ...
                                if menu_condo.is_visible(timeout=2000):
                                    log(f"   🏢 Saindo do condomínio atual...")
                                    menu_condo.click(timeout=5000)
                                    page.wait_for_timeout(1000)

                                    page.get_by_role("menuitem", name="Sair").click(timeout=5000)
                                    page.wait_for_timeout(1500)
                                    log(f"   ✅ Voltou para área de seleção de condomínios")
                                else:
                                    log(f"   ℹ️  Já está na área de seleção")

                            except Exception as e:
                                log(f"   ⚠️ Erro ao sair do condomínio: {e}")
                ...
                # ===== LÓGICA PADRÃO PARA OUTRAS PLATAFORMAS ===== ↓
                    ...
                    if prev_creds is None and family_now == "livefacilities":
                        ...
                        # log(f"   on_login_page={on_login_page}, esta_em_blank={esta_em_blank}, ja_logado={ja_logado}")
