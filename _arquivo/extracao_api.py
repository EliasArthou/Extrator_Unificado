...
class FixedProgressLogger:
    ...
    def start(self):
        self.live = Live(
            self._build_display(),
            console=console,
            refresh_per_second=0,   # sem auto-refresh; atualiza só em advance()/update_description()
            transient=False,
            screen=False,
...
try:
    ...
    import boletos as _boletos_mod
    _tem_aux = True
except ImportError:
    _tem_aux = False
...
def _familia_from_url(url: str) -> str:
    ...
    if "superlogica" in u:
        return "superlogica"
    if "livefacilities" in u or "bap.com.br" in u or "app.bcfadm.com.br" in u or "fffacilities.com.br" in u or "zirtaweb.com" in u:
...
FAMILIAS_SUPORTADAS = {"superlogica", "livefacilities", "webware", "protel", "nacional", "cipa", "condomob"}
...
class SessaoLiveFacilities:
    ...
    def listar_unidades(self) -> list[dict]:
        ...
        Cada item: {condo, bloco, unidade, postback_target}
        """
    ...
    def selecionar_unidade(self, unidade_dict: dict, soup_pagina):
        ...
        if not action or not action.startswith("http"):
            action = urljoin(self.base_url, action or "/")
    ...
    def listar_boletos(self, url_boletos: str) -> list[dict]:
        """
        Acessa a página de lista de boletos e retorna hrefs dos PDFs.
        O seletor original (Playwright) é:
          #ContentBody_ucBoletoLista_upListaBoleto > div.content:not(...) a[title='Abrir Boleto']
        Aqui extraímos qualquer link com /pCli_BoletoNovo.aspx (dentro do iframe do modal).
        """
        ...
        # Coleta hrefs dos botões "Abrir Boleto" (cada um leva a um modal com iframe)
        botoes = soup.find_all(
            "a", title="Abrir Boleto",
            attrs={"href": re.compile(r"pCli_BoletoNovo", re.I)}
        )
        if not botoes:
            # Tenta encontrar links diretos ao PDF
            botoes = soup.find_all("a", href=re.compile(r"pCli_BoletoNovo", re.I))

        return [{"pdf_url": urljoin(self.base_url, b["href"])} for b in botoes]
...
def processar(
    ...
    for linha in linhas_ok:
        chave = (
            str(linha[3]).strip(),
            str(linha[1]).strip(),   # credenciais já stripped aqui
            str(linha[2]).strip(),
        )
        grupos[chave].append(linha)

    for (nome_adm, email, senha), grupo in grupos.items():
        site_url = MAPA_SITES.get(nome_adm, "")
        familia  = _familia_from_url(site_url)

        if progress:
            progress.update_description(
                f"[green]{nome_adm[:35]}... ({familia})"
            )

        log(f"\n{'='*70}")
        log(f"  {nome_adm}  [{familia}]  |  {email}  |  {len(grupo)} registro(s)")
        log(f"{'='*70}")

        # ── Credenciais vazias → não tenta login, já encerra aqui ─────────────
        _cred_vazia = not email and not senha
        _login_vazio = not email
        _senha_vazia = not senha
        if _login_vazio or _senha_vazia:
            ...
            log(f"  ⚠ SEM CREDENCIAL — {_aviso}. Extração ignorada.")
            for l in grupo:
        ...
        try:
            if familia == "superlogica":
                resultados += _processar_superlogica(
                    _base_url(site_url), email, senha, grupo, log
                )
                ...
                resultados += _processar_webware(
                    site_url, email, senha, grupo, log
                )
            elif familia == "protel":
                resultados += _processar_protel(
                    email, senha, grupo, log
                )
            ...
            log(f"  [ERRO INESPERADO] {nome_adm} | {type(exc).__name__}: {exc}")
            for l in grupo:
...
def _processar_livefacilities(base_url, usuario, senha, grupo, log) -> list:
    ...
    for linha in grupo:
        ...
        login_unico   = not linha[6]  # LOGINMULTIPLO=0 → login dedicado a esta unidade
        ...
        if login_unico:
            ...
            # Fuzzy match de unidade
            # validar_numero=False: textos do site têm prefixo de código (ex: '1210 - Nome Apto 208')
            # cujo nº de dígitos supera o nº do apartamento, zerando o score incorretamente
            textos = [u["texto"] for u in unidades]
            idx, score = _melhor_match_score(alvo_texto, textos, cutoff=0.60, validar_numero=False)
            if idx is None:
        ...
        if login_unico and not baixou_algum:
            log(f"  [{identificador}] Sem boletos em aberto ({unidades_verificadas} unidade(s) verificada(s))")
...
def _processar_condomob(site_url: str, email: str, senha: str, grupo: list, log) -> list:
    ...
    for linha in grupo:
        ...
        unidades_verificadas = 0   # total de unidades checadas (para log de sem_boleto)
        baixou_algum       = False
...
def _salvar_pdf_em_avaliacao(pdf_bytes: bytes, identificador: str, log) -> dict:
    """
    Salva PDF em DOWNLOADS_DIR/Em avaliação/{identificador}.pdf quando a unidade
    não tem nome no site (não foi possível fazer o match).
    Usa sufixo _n se o arquivo já existir.
    """
    pasta = DOWNLOADS_DIR / "Em avaliação"
...
def _salvar_pdf(pdf_bytes: bytes, identificador: str, mensagem_ok: str,
    ...
    destino = DOWNLOADS_DIR / f"{nome_base}.pdf"
    k = 0
    while destino.exists():
        k += 1
        destino = DOWNLOADS_DIR / f"{nome_base}_{k}.pdf"
    ...
    # ── Move para subpasta do mês + extrai código de barras ───────────────────
    # (processar_boleto_baixado renomeia e move independentemente do fluxo-pw)
    ...
    if _tem_aux:
        try:
            ...
            sys.stdout = _null          # suprime prints de boletos.py
            try:
            ...
            if dados:
                ...
                log(f"    Movido: {caminho_final.parent.name}/{caminho_final.name}")
        except Exception as e:
...
def _pasta_ciclo(ano_mes: "str | None") -> Path:
    ...
    if ano_mes:
        ...
        if len(partes) == 2 and partes[0].isdigit() and partes[1].isdigit():
            ...
            if 1 <= mes <= 12:
                return DOWNLOADS_DIR / f"{ano}_{mes:02d}_{_MESES_PT[mes - 1]}"
        raise ValueError(f"Formato inválido para ciclo: '{ano_mes}' — use YYYY_MM (ex: 2026_04)")
    # fallback: mês atual
    from datetime import datetime as _dt
    hoje = _dt.now()
    return DOWNLOADS_DIR / f"{hoje.year}_{hoje.month:02d}_{_MESES_PT[hoje.month - 1]}"
