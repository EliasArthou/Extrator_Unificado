...
from sensiveis import retornaradministradora
...
class ExecutorFluxoPW:
    ...
    def executar(self, passos, linha):
        for indice, passo in enumerate(passos):
            try:
                if indice == 7:
                    w = 1
                print(indice, passo)
                self._executar_acao(passo, linha)
            except Exception as e:
                self.resposta = str(e)
                self.passo_interrompido = passo
                break

    def get_status(self, chave: str = None):
        """Retorna um snapshot do estado final da execução ou um item específico se solicitado."""
        status = {
    ...
    def reset_status(self):
        """Opcional: zera mensagens/flags antes de uma nova execução."""
        self.resposta = ""
        self.mensagemerro = ""
        self.passo_interrompido = None

    def _obter_elemento(self, seletor: str, tipo="css", esperar=False):
        if esperar:
            self.page.wait_for_selector(seletor, timeout=5000)

        if tipo == "css":
            el = self.page.query_selector(seletor)
        elif tipo == "xpath":
            el = self.page.locator(f'xpath={seletor}').element_handle()
        else:
            raise ValueError(f"Tipo de seletor não suportado: {tipo}")

        return el

    def _executar_acao(self, passo, linha):
        acao = passo.get("acao")
        seletor = passo.get("seletor")
        esperar = passo.get("esperar", False)
        interromper = passo.get("interromper_se_nao_achar", False)
        interromper_se_achar = passo.get("interromper_se_achar", False)

        match acao:
            case "verificar_elemento":
                el = self._obter_elemento(seletor, esperar=esperar)

                if el:
                    self.mensagemerro = el.inner_text().strip()
                    self.resposta = f"[verificar_elemento] Elemento '{seletor}' exibido com texto: '{self.mensagemerro}'"
                    if interromper_se_achar:
                        raise ValueError(f"[verificar_elemento] Interrupção solicitada. Texto exibido: '{self.mensagemerro}'")
                elif interromper:
                    raise ValueError(f"[verificar_elemento] Elemento '{seletor}' não encontrado.")
                else:
                    self.resposta = f"[verificar_elemento] Elemento '{seletor}' não encontrado, mas continuação permitida."


            case "goto":
                self.page.goto(passo["valor"])

            case "click":
                el = self._obter_elemento(seletor, esperar=esperar)
                if el:
                    if interromper_se_achar:
                        raise ValueError(f"Elemento '{seletor}' encontrado e interrupção solicitada")
                    el.click()
                elif interromper:
                    raise ValueError(f"Elemento '{seletor}' não encontrado")
            ...
            case "fill":
                ...
                if el:
                    if interromper_se_achar:
                        raise ValueError(f"Elemento '{seletor}' encontrado e interrupção solicitada")
                    valor = self._resolver_variavel(passo["valor"], linha)
                    el.fill(valor)
                elif interromper:
                    raise ValueError(f"Elemento '{seletor}' não encontrado")

            case "hover":
                el = self._obter_elemento(seletor, esperar=True)
                if el:
                    if interromper_se_achar:
                        raise ValueError(f"Elemento '{seletor}' encontrado e interrupção solicitada")
                    el.hover()
            ...
            case "listar_elementos":
                if esperar:
                    self.page.wait_for_selector(seletor, timeout=5000)
                elementos = self.page.query_selector_all(seletor)
                for el in elementos:
                    print(" -", el.inner_text().strip())

            case "selecionar_por_texto":
                valor = self._resolver_variavel(passo["valor"], linha)
                cutoff = passo.get("cutoff", 0.85)
                self._selecionar_item_por_texto(seletor, valor, cutoff, interromper, interromper_se_achar)

            case "hover_e_click_proximo":
                hover = passo["hover"]
                texto = self._resolver_variavel(passo["texto_proximo"], linha)
                cutoff = passo.get("cutoff", 0.85)
                self._hover_e_selecionar_proximo(hover, seletor, texto, cutoff, interromper, interromper_se_achar)

    def _selecionar_item_por_texto(self, seletor, valor, cutoff=0.8, interromper=False, interromper_se_achar=False,
                                   separador="/"):
        if isinstance(seletor, list):
            if len(seletor) < 2:
                raise ValueError(
                    "Se estiver usando lista de seletores, deve haver pelo menos dois: um para comparação e um para clique.")

            seletores_comparacao = seletor[:-1]
            seletor_click = seletor[-1]

            listas_elementos = [self.page.query_selector_all(sel) for sel in seletores_comparacao]
            elementos_click = self.page.query_selector_all(seletor_click)

            tamanho = len(elementos_click)
            if not all(len(lista) == tamanho for lista in listas_elementos):
                raise ValueError("Todas as listas de elementos devem ter o mesmo tamanho.")

            partes_valor = [v.strip() for v in valor.split(separador)]
            if len(partes_valor) != len(seletores_comparacao):
                raise ValueError(
                    f"Esperado {len(seletores_comparacao)} partes separadas por '{separador}', mas recebeu {len(partes_valor)}.")

            melhor_score = 0
            melhor_indice = -1

            for i in range(tamanho):
                partes_elemento = [lista[i].inner_text().strip() for lista in listas_elementos]
                texto_item = separador.join(partes_elemento)
                texto_busca = separador.join(partes_valor)

                score = fuzz.ratio(texto_item.upper(), texto_busca.upper()) / 100.0
            ...
            if melhor_score >= cutoff:
                if interromper_se_achar:
                    raise ValueError(
                        f"Item '{valor}' encontrado com score {melhor_score:.2f} e interrupção solicitada.")
                elementos_click[melhor_indice].click()
            elif interromper:
                raise ValueError(f"Item '{valor}' não encontrado com similaridade >= {cutoff}")
        else:
            # Modo clássico
            elementos = self.page.query_selector_all(seletor)
            candidatos = [(el, el.inner_text().strip()) for el in elementos]

            melhor_score = 0
            melhor_elemento = None

            for el, txt in candidatos:
                score = fuzz.ratio(txt.upper(), valor.upper()) / 100.0
                if score > melhor_score:
                    melhor_score = score
                    melhor_elemento = el
            ...
            if melhor_score >= cutoff:
                if interromper_se_achar:
                    raise ValueError(f"Item '{valor}' encontrado com score {melhor_score:.2f} e interrupção solicitada")
                self.page.get_by_role("link", name=melhor_elemento).click()
                # melhor_elemento.click()
            elif interromper:
                raise ValueError(f"Item '{valor}' não encontrado com similaridade >= {cutoff}")

    def _resolver_variavel(self, valor, linha):
        if isinstance(valor, str) and valor.startswith("${") and valor.endswith("}"):
            chave = valor[2:-1]
            return linha.get(chave, valor)
        return valor
